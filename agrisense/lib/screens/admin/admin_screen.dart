...
class _LessonsTabState extends State<_LessonsTab> {
  ...
  Future<void> _load() async {
    ...
    final fs = context.read<AppProvider>().firestore;
    _lessons = await fs.getAllLessonsAdmin();
  ...
  void _openLessonForm(BuildContext context, LessonData? lesson) async {
    final result = await Navigator.push<bool>(context,
        MaterialPageRoute(builder: (_) => LessonFormScreen(lesson: lesson)));
    if (result == true) _load();
  }

  Future<void> _deleteLesson(LessonData l) async {
    ...
    if (confirm == true) {
      await context.read<AppProvider>().firestore.deleteLesson(l.id);
      _load();
    }
  }

  Future<void> _toggleActive(LessonData l) async {
    final fs = context.read<AppProvider>().firestore;
    await fs.saveLesson(LessonData(
      id: l.id, title: l.title, titleKin: l.titleKin,
      cropTag: l.cropTag, topicTag: l.topicTag, level: l.level,
      formats: l.formats, durationMinutes: l.durationMinutes, emoji: l.emoji,
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('${lesson.topicTag} · ${lesson.durationMinutes} min · ${lesson.level}',
...
class _LessonFormScreenState extends State<LessonFormScreen> {
  ...
  Future<void> _save() async {
    ...
    final data = LessonData(
      ...
      level: _level, formats: _formats.isEmpty ? ['text'] : _formats,
      isNew: _isNew, isWomensPathway: _isWomens,
      isActive: true, order: l?.order ?? 999,
    );

    final fs = context.read<AppProvider>().firestore;
    final id = await fs.saveLesson(data);
    if (mounted) {
...
class _TipsTabState extends State<_TipsTab> {
  ...
  Future<void> _load() async {
    setState(() => _loading = true);
    _tips = await context.read<AppProvider>().firestore.getAllTipsAdmin();
    if (mounted) setState(() => _loading = false);
  }
  ...
  Future<void> _deleteTip(String id) async {
    await context.read<AppProvider>().firestore.saveTip(
        id: id, title: '', titleKin: '', body: '', bodyKin: '', emoji: '');
    // Soft delete via isActive=false workaround — or use a real delete:
...
class _CommunityTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<CommunityPostData>>(
          ...
          itemBuilder: (_, i) {
            ...
            return Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('${p.upvoteCount} upvotes · ${p.replyCount} replies · ${p.district}',
                    style: const TextStyle(fontSize: 10, color: AppColors.gray400)),
              ]),
            );
          },
