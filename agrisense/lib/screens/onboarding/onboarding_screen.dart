...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  Future<void> _verifyOtp(String code) async {
    ...
    // ── Test mode bypass (no Firebase required) ──────────────────────────
    if (auth.isTestMode) {
      await Future.delayed(const Duration(milliseconds: 600)); // simulate verify
      if (!mounted) return;
      if (code == AuthService.testCode) {
        await context.read<AppProvider>().completeOnboarding(
          name: _farmerName.trim().isNotEmpty ? _farmerName.trim() : 'Test Farmer',
        ...
        setState(() { _otpVerifying = false; _otpStatusMsg = 'Wrong code. Use ${AuthService.testCode}'; });
      }
      return;
    }
        ...
        child: Column(children: [
          ...
          Expanded(
            child: PageView(
              ...
              children: [
                // 0 ── Splash
                _SplashPage(onGetStarted: _next),
...
class _SplashPage extends StatelessWidget {
  ...
  const _SplashPage({required this.onGetStarted});
